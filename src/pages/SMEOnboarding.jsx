...
export default function SMEOnboarding() {
    ...
    return (
        <div className="min-h-screen bg-[var(--cream)] font-sans flex flex-col items-center justify-center p-6">
                ...
                {step === 2 && (
                    <div className="animate-fade-in text-center">
                        ...
                        <div className="grid md:grid-cols-3 gap-6 text-left mb-10">
                            ...
                            {/* Card 1 */}
                            <button
                                onClick={() => handleSelectFramework('bank')}
                                className={`bg-[var(--white)] rounded-2xl p-6 border-2 transition relative flex flex-col h-full hover:shadow-md ${selectedFramework === 'bank' ? 'border-[var(--forest)] shadow-md' : 'border-gray-200'}`}
                            >
                                {selectedFramework === 'bank' && (
                                    <div className="absolute top-4 right-4 text-[var(--forest)] text-xl bg-[var(--light-green)] rounded-full w-8 h-8 flex items-center justify-center">✓</div>
                                )}
                                <div className="text-4xl mb-4">🏦</div>
                                <h3 className="font-bold text-xl text-[var(--dark)] mb-2">{frameworks.bank.label}</h3>
                                <p className="text-[var(--gray)] text-sm mb-6">{frameworks.bank.description}</p>
                                <ul className="space-y-3 mt-auto text-sm text-[var(--dark)] font-medium">
                                    <li className="flex items-start gap-2"><span className="text-[var(--mint)]">✓</span> Energy and emissions data</li>
                                    <li className="flex items-start gap-2"><span className="text-[var(--mint)]">✓</span> Workforce information</li>
                                    <li className="flex items-start gap-2"><span className="text-[var(--mint)]">✓</span> Basic governance policies</li>
                                </ul>
                                <div className={`mt-6 py-2 px-4 rounded font-bold text-center w-full transition ${selectedFramework === 'bank' ? 'bg-[var(--forest)] text-white' : 'bg-gray-100 text-gray-700'}`}>
                                    {selectedFramework === 'bank' ? 'Selected' : 'Select'}
                                </div>
                            </button>

                            {/* Card 2 */}
                            <button
                                onClick={() => handleSelectFramework('customer')}
                                className={`bg-[var(--white)] rounded-2xl p-6 border-2 transition relative flex flex-col h-full hover:shadow-md ${selectedFramework === 'customer' ? 'border-[var(--forest)] shadow-md' : 'border-gray-200'}`}
                            >
                                {selectedFramework === 'customer' && (
                                    <div className="absolute top-4 right-4 text-[var(--forest)] text-xl bg-[var(--light-green)] rounded-full w-8 h-8 flex items-center justify-center">✓</div>
                                )}
                                <div className="text-4xl mb-4">🤝</div>
                                <h3 className="font-bold text-xl text-[var(--dark)] mb-2">{frameworks.customer.label}</h3>
                                <p className="text-[var(--gray)] text-sm mb-6">{frameworks.customer.description}</p>
                                <ul className="space-y-3 mt-auto text-sm text-[var(--dark)] font-medium">
                                    <li className="flex items-start gap-2"><span className="text-[var(--mint)]">✓</span> Scope 1 & 2 emissions</li>
                                    <li className="flex items-start gap-2"><span className="text-[var(--mint)]">✓</span> Supply chain information</li>
                                    <li className="flex items-start gap-2"><span className="text-[var(--mint)]">✓</span> Social and governance policies</li>
                                </ul>
                                <div className={`mt-6 py-2 px-4 rounded font-bold text-center w-full transition ${selectedFramework === 'customer' ? 'bg-[var(--forest)] text-white' : 'bg-gray-100 text-gray-700'}`}>
                                    {selectedFramework === 'customer' ? 'Selected' : 'Select'}
                                </div>
                            </button>

                            {/* Card 3 */}
                            <button
                                onClick={() => handleSelectFramework('vsme')}
                                className={`bg-[var(--white)] rounded-2xl p-6 border-2 transition relative flex flex-col h-full hover:shadow-md ${selectedFramework === 'vsme' ? 'border-[var(--forest)] shadow-md' : 'border-gray-200'}`}
                            >
                                {selectedFramework === 'vsme' && (
                                    <div className="absolute top-4 right-4 text-[var(--forest)] text-xl bg-[var(--light-green)] rounded-full w-8 h-8 flex items-center justify-center">✓</div>
                                )}
                                <div className="text-4xl mb-4">📋</div>
                                <h3 className="font-bold text-xl text-[var(--dark)] mb-2">{frameworks.vsme.label}</h3>
                                <p className="text-[var(--gray)] text-sm mb-6">{frameworks.vsme.description}</p>
                                <ul className="space-y-3 mt-auto text-sm text-[var(--dark)] font-medium">
                                    <li className="flex items-start gap-2"><span className="text-[var(--mint)]">✓</span> Full environmental disclosure</li>
                                    <li className="flex items-start gap-2"><span className="text-[var(--mint)]">✓</span> Workforce and social data</li>
                                    <li className="flex items-start gap-2"><span className="text-[var(--mint)]">✓</span> Governance information</li>
                                </ul>
                                <div className={`mt-6 py-2 px-4 rounded font-bold text-center w-full transition ${selectedFramework === 'vsme' ? 'bg-[var(--forest)] text-white' : 'bg-gray-100 text-gray-700'}`}>
                                    {selectedFramework === 'vsme' ? 'Selected' : 'Select'}
                                </div>
                            </button>
